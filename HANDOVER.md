...
**尚未修（複檢報告的第二、三波）**：訂單追蹤訂金算法未同步新版（#3，前端 05_orders.js＋後端 v2_extensions.gs）、自動費用列 auto 標記存讀遺失（#5）、toggleCol 掉 pid 級距失效（#6）、後端 updateQuote 整表清空（#4）、自訂單雙引號毀損（#7）、驗收同名同容量重複計（#8）等，清單詳見複檢報告。
