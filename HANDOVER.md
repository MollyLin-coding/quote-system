...
### E-2. LOGO 待辦（v1.2 新增，下一個接手對話優先看這裡）
Molly 提供的 logo 檔案目前是純黑色無內容的圖片（無法使用），**等 Molly 補上正確的透明背景 LOGO 檔案後才能套用**，詳見「四點六」章節的「LOGO 待辦」說明。
...
| # | 需求 | 修正方式 | 程式位置 |
|---|---|---|---|
| 1 | 左上 LOGO 換成公司標楷 LOGO | **未完成，待補檔**（見下方「LOGO 待辦」說明） | `openPreview()` header 區塊 |
...
### LOGO 待辦（重要，下一個接手對話請先看這裡）

- Molly 在這次對話上傳過一個「公司標楷 LOGO」檔案（`logo_black.png`），**經實際檢查該檔案為 1520×236px、RGB 模式（無透明背景）、全部像素皆為純黑 RGB(0,0,0)，等同一張純黑色矩形，沒有任何可辨識的 LOGO 圖案**。
- **這極可能就是「LOGO 一直沒套用成功」的根本原因**：之前的接手對話收到的應該是同樣有問題的檔案，貼上等於貼一塊黑色方塊，因此判斷無法使用而沒套用——不是疏漏或沒看說明，是來源檔案本身沒有可用內容。
- **下一步**：請 Molly 重新提供「透明背景的 PNG」（LOGO 線稿為深色，背景透明）或原始向量檔（AI/SVG/EPS）。拿到有效檔案後，LOGO 套用方式：
  1. 把檔案 push 到 repo（例如 `assets/logo.png`），用 `<img src="https://raw.githubusercontent.com/MollyLin-coding/quote-system/main/assets/logo.png">` 嵌入 `openPreview()` header 區塊，取代目前純 CSS 畫的金色直線＋文字 logo-mark。
  2. **拿到檔案後務必先用本節「檢查圖片是否為純色/透明背景」的方法（PIL 讀取 mode + getextrema()）驗證過一次，確認真的有可見內容，再動手套用**，避免重蹈覆轍。
  3. 套用後記得同步調整 header 區塊版面（圖片 + 公司名文字並排的 gap、對齊）。
