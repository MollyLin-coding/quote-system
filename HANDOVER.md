...
# 四點二十：2026-08-11 優化第二波（每日待辦信＋卡關天數＋應收帳齡）

複檢報告的「優化建議」#2 與 #3。Molly 挑的：先做這兩個，權限那批（第二級）之後再做。
回歸測試＝`test_wave6.js`（39 項全過）。

## #2 每日 08:00 把今日待辦寄到信箱（`gas/v33_digest.gs`）
LINE 還沒申辦，先做 Gmail 版；摘要文字直接沿用既有的 `digestTextSummary_`
（那支 2026-07-25 就寫好了，一直註明「未接線」）。之後要接 LINE 只要換掉送出那一行。

| 函式 | 用途 |
|---|---|
| `sendDailyDigestMail()` | 觸發器進入點。**名稱不帶底線**，否則觸發器掛不上（同 `runCalendarSync` 慣例）。 |
| `setupDailyDigestMail()` | 掛（或重掛）每天 08:00 的觸發器；會先刪掉同 handler 的舊觸發器，重複執行不會長出第二個。 |
| `previewDailyDigestMail()` | 只寫進執行記錄，**不寄信**。 |
| `removeDailyDigestMail()` | 停掉每日信。 |

- **收件人**：Script Property `DIGEST_MAIL_TO`；沒設就 `Session.getEffectiveUser().getEmail()`（＝專案擁有者本人）。
  刻意不寫死信箱，換人接手不用改碼。
- **暫停**：Script Property `DIGEST_MAIL_OFF=YES`（觸發器留著、只是不寄；刪掉屬性就恢復）。
- **主旨**帶件數：`【南坡萬】08/12 今日待辦：該出貨 2、催尾款 1`——手機通知列一眼看完，不用點開。
  0 件的類別不列；全都沒有就寫「今天沒事 ☕」。件數只算 `urgent !== false` 的，跟推播摘要一致。
- **⚠ 沒事的日子也照寄**。這是刻意的：沒收到信＝系統壞了，有收到信＝今天真的沒事，
  兩者要分得開。想改成「沒事就不寄」的話，`sendDailyDigestMail` 裡加一行早退即可。
- **待辦讀不出來時會另外寄一封「產生失敗」通知**，不會安靜地不寄。
- ⚠ Google 的時間觸發器是「那個小時內」，不是 08:00 整。

**Molly 要自己做一次**（Claude 不按「執行」）：編輯器函式下拉選 `setupDailyDigestMail` → 執行 → 首次授權按允許。
執行記錄會寫「已設定每天 8:00 前後寄出到 xxx」。想先看信長什麼樣就先跑 `previewDailyDigestMail`。

## #3 訂單列「停在〈某關卡〉N 天」（`js/05_orders.js`）
- `ordStageSince(o)`：取「讓它進到目前這一關的那個日期」往今天算——
  報價中→報價日、已收訂金/排產中→訂金日、已出貨→實際出貨日、已開發票→發票日（沒有就退實際出貨日）、
  已收尾款→尾款收款日。**結案與取消回 null**（已經結束，不需要催）；日期缺漏也回 null。
- `ordStuckBadge(o)`：**7 天內不顯示**（正常流程不吵）；7–13 天藍、14–29 天橘、30 天以上紅。
  掛在 `orderBadges` 最前面。

## #3 月報表未收尾款：帳齡（`js/05_orders.js`＋`assets/app.css`）
- `rptAgeDays(o)`：發票日 →（沒有）實際出貨日 →（沒有）預計尾款日 →（沒有）報價日；都沒有回 null。
  用發票日當首選是因為「開了票才真的算應收」。
- 表格**最後**加一欄「帳齡」，並改成**欠最久的排最前面**（原本是照預計尾款日）。
  ⚠ 新欄位刻意加在最後、統計卡維持 6 張——`test_rpt.js` 有 `T-B[\s\S]{0,400}逾期` 這種
  「往後數 400 字」的比對，欄位插在中間會把它擠出範圍。合計列 `colspan` 已從 2 改 3。
- 表格上方加帳齡分組卡（≤30／31–60／61–90／>90／無日期），各自顯示金額與筆數，空的組不顯示。
  樣式 `.rpt-age` / `.rpt-age-box` 在 app.css，860px 以下兩欄。

## 回歸狀況
`test_wave6.js` 39 項全過。全專案 40 支與 `git stash` 基準線逐支比對，**零新增失敗**。
⚠ 這批測試在同一台機器連續跑會互相搶瀏覽器而**偶發假失敗**（本次 `test_calmark`／`test_ord_v32`
就各出現過一次，單獨重跑即 0）。判定新增失敗前務必單獨重跑一次再跟 `git stash` 基準線比。
